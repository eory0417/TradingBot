"""Telethon 기반 @coinnesskr 텔레그램 채널 실시간 수신 (Plus 전용 뉴스 소스).
...
수신한 한국어 메시지는 :class:`news_analyzer.NewsItem` 으로 정규화되어
(``origin="coinnesskr"``, ``title_ko`` 에 원문 보관) 공통 파이프라인에서
KO→EN 번역 후 FinBERT 점수화된다.
"""
...
def parse_coinness_message(message) -> NewsItem | None:
    ...
    if title_match:
        ...
        title_ko = first_line[0].strip() if first_line else ""
    title_ko = title_ko[:_MAX_TITLE_CHARS].strip()
    if not title_ko:
        return None
    ...
    return NewsItem(
        ...
        title=title_ko,        # 파이프라인에서 EN 번역본으로 덮어쓴다.
        url=url,
        source="coinnesskr",
...
class CoinnessListener:
    """``@coinnesskr`` 채널을 Telethon 유저 세션으로 수신한다.
    ...
    async def connect(self) -> bool:
        ...
        if not self._credentials_ok():
            log.warning(
                "coinnesskr 비활성: TELEGRAM_API_ID/HASH 미설정 — .env 를 확인하세요."
            )
            return False
        session_path = settings.telegram_session_path
        if not session_path.exists():
            log.warning(
                "coinnesskr 세션 없음 (%s) — 먼저 `python telegram_login.py` 를 실행하세요.",
                session_path,
            )
            return False
        ...
        try:
            ...
            log.warning("coinnesskr 비활성: telethon 미설치 (pip install telethon)")
            return False
        ...
        try:
            ...
            if not await self._client.is_user_authorized():
                log.warning(
                    "coinnesskr 세션 미인증 — `python telegram_login.py` 를 다시 실행하세요."
                )
        ...
        log.info("coinnesskr 연결됨 | channel=@%s", self._channel)
        return True
